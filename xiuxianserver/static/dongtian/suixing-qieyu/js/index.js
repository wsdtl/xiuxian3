...
// Gameplay
const getSpawnDelay = () => {
	const spawnDelayMax = 720;
	const spawnDelayMin = 260;
	const spawnDelay = spawnDelayMax - state.game.cubeCount * 2.2;
	const tideBoost = window.SUIXING_TIDE_ACTIVE ? 0.48 : 1;
	return Math.max(spawnDelay * tideBoost, window.SUIXING_TIDE_ACTIVE ? 120 : spawnDelayMin);
}
const doubleStrongEnableScore = 900;
// Number of cubes that must be smashed before activating a feature.
const slowmoThreshold = 4;
const strongThreshold = 10;
const spinnerThreshold = 8;
...
const emitTrialEvent = (name, detail={}) => {
	window.dispatchEvent(new CustomEvent(`suixing:${name}`, { detail }));
};
...
const getTarget = (() => {
	...
	const strongSpawner = makeSpawner({
		chance: 0.55,
		cooldownPerSpawn: 5200,
		maxSpawns: 2
	});

	const spinnerSpawner = makeSpawner({
		chance: 0.34,
		cooldownPerSpawn: 3600,
		maxSpawns: 2
	});
...
function renderScoreHud() {
	scoreNode.innerText = `玉尘 ${state.game.score}`;
	scoreNode.style.display = 'block';
	cubeCountNode.style.opacity = 0.78;
	cubeCountNode.innerText = `切玉 ${state.game.cubeCount}｜连斩 ${state.game.combo}`;
}
...
function renderMenus() {
	...
	switch (state.menus.active) {
		...
		case MENU_SCORE:
			...
			highScoreLblNode.textContent = `切玉 ${state.game.cubeCount}｜最高连斩 ${state.game.maxCombo}｜漏失 ${state.game.misses}`;
...
function setScore(score) {
	...
	renderScoreHud();
	emitTrialEvent('score', snapshotTrial());
}
...
function resetGame() {
	...
	state.game.combo = 0;
	state.game.misses = 0;
	state.game.strongCubes = 0;
	state.game.slowmoCubes = 0;
	state.game.endReason = 'manual';
	state.game.roundStartedAt = Date.now();
	resetAllCooldowns();
	setScore(0);
	setCubeCount(0);
	spawnTime = getSpawnDelay();
	emitTrialEvent('start', snapshotTrial());
}
...
function endGame(reason='manual') {
	...
	emitTrialEvent('finish', snapshotTrial());
}

function snapshotTrial() {
	return {
		score: state.game.score,
		cubesSliced: state.game.cubeCount,
		strongCubes: state.game.strongCubes,
		slowmoCubes: state.game.slowmoCubes,
		misses: state.game.misses,
		maxCombo: state.game.maxCombo,
		combo: state.game.combo,
		elapsedSeconds: Math.max(0, Math.round(state.game.time / 1000)),
...
function registerMiss() {
	...
	state.game.combo = 0;
	incrementScore(-18);
	emitTrialEvent('miss', snapshotTrial());
}

function registerSlice(target, pointerSpeedScaled) {
	if (!isInGame()) return;
	state.game.combo += 1;
	if (state.game.combo > state.game.maxCombo) {
		state.game.maxCombo = state.game.combo;
	}
	if (target && target.maxHealth > 1) {
		state.game.strongCubes += 1;
	}
	if (target && target.wireframe) {
		state.game.slowmoCubes += 1;
	}
	emitTrialEvent('slice', {
		...snapshotTrial(),
		wireframe: !!(target && target.wireframe),
		strong: !!(target && target.maxHealth > 1),
		pointerSpeed: Math.round(pointerSpeedScaled || 0)
	});
}
...
const slowmoDuration = 900;
let slowmoRemaining = 0;
let spawnExtra = 0;
const spawnExtraDelay = 115;
let targetSpeed = 1;
...
function tick(width, height, simTime, simSpeed, lag) {
	...
	if (slowmoRemaining > 0) {
		...
		targetSpeed = pointerIsDown ? 0.18 : 0.54;
	} else {
		const menuPointerDown = isMenuVisible() && pointerIsDown;
		targetSpeed = menuPointerDown ? 0.05 : (window.SUIXING_TIDE_ACTIVE ? 1.35 : 1.12);
	}
	...
	if (spawnTime <= 0) {
		...
		const tideSpeed = window.SUIXING_TIDE_ACTIVE ? 1.28 : 1;
		target.xD = (Math.random() * (target.x * -2 / 95)) * tideSpeed;
		target.yD = (-24 - Math.random() * 5) * tideSpeed;
		targets.push(target);
		if (window.SUIXING_TIDE_ACTIVE && Math.random() < 0.52) {
			spawnExtra += 1;
		}
	}
				...
				if (!target.hit) {
					...
					if (pointerSpeedScaled > minPointerSpeed) {
						...
						if (target.health <= 0) {
							...
							registerSlice(target, pointerSpeedScaled);
							createBurst(target, forceMultiplier);
							sparkBurst(hitX, hitY, 8, sparkSpeed);
							if (target.wireframe) {
								slowmoRemaining = slowmoDuration;
								spawnTime = 0;
								spawnExtra = window.SUIXING_TIDE_ACTIVE ? 5 : 3;
							}
							targets.splice(i, 1);
							returnTarget(target);
						} else {
...
function setupCanvases() {
	...
	function handleResize() {
		...
		const w = Math.max(320, Math.floor(viewport ? viewport.width : window.innerWidth));
		const h = Math.max(480, Math.floor(viewport ? viewport.height : window.innerHeight));
		viewScale = h / 1000;
		width = w / viewScale;
		height = h / viewScale;
		canvas.width = w * dpr;
